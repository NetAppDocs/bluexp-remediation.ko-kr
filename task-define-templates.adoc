...
* Azure NetApp Files 볼륨 템플릿을 생성할 때 사용자에게 Azure NetApp Files 작업 환경을 사용할 수 있는지 확인합니다. 자세한 내용은 를 참조하십시오 https://docs.netapp.com/us-en/cloud-manager-azure-netapp-files/task-manage-anf.html["Azure NetApp Files 작업 환경을 만듭니다"^] 않습니다.
...
을 참조하십시오 https://docs.netapp.com/us-en/cloud-manager-cloud-volumes-ontap/task-provisioning-storage.html#creating-flexvol-volumes["Cloud Volumes ONTAP 볼륨 프로비저닝 방법"^] Cloud Volumes ONTAP 볼륨 템플릿에서 완료해야 하는 모든 매개변수에 대한 자세한 내용은 를 참조하십시오.
...
. Working Environment(작업 환경) 및 Volume Name(볼륨 이름) 필드 아래에는 백업을 활성화할 볼륨을 나타내는 세 가지 선택 항목이 있습니다. 을 참조하십시오 <<Pass values between template actions,이 필드를 작성하는 방법>>.
